...
const DataVisualization = () => {
  ...
  const generateVisualizationData = () => {
    if (!liveDemo.analyticsData.chartData || liveDemo.analyticsData.chartData.length === 0) {
      return [];
    }
...
plt.title('${selectedMetric.charAt(0).toUpperCase() + selectedMetric.slice(1)} by Category')
plt.xlabel('Category')
plt.ylabel('${selectedMetric.charAt(0).toUpperCase() + selectedMetric.slice(1)}')
plt.xticks(rotation=45)` : ''}

${chartType === 'line' ? `# Line chart
plt.plot(df['timestamp'], df['${selectedMetric}'], marker='o')
plt.title('${selectedMetric.charAt(0).toUpperCase() + selectedMetric.slice(1)} Over Time')
plt.xlabel('Time')
plt.ylabel('${selectedMetric.charAt(0).toUpperCase() + selectedMetric.slice(1)}')
plt.xticks(rotation=45)` : ''}
...
plt.title('${selectedMetric.charAt(0).toUpperCase() + selectedMetric.slice(1)} Distribution')` : ''}

${chartType === 'scatter' ? `# Scatter plot
plt.scatter(df['value'], df['${selectedMetric}'], alpha=0.7)
plt.title('${selectedMetric.charAt(0).toUpperCase() + selectedMetric.slice(1)} vs Value')
plt.xlabel('Value')
plt.ylabel('${selectedMetric.charAt(0).toUpperCase() + selectedMetric.slice(1)}')` : ''}
...
${chartType === 'bar' ? `fig = px.bar(df, x='category', y='${selectedMetric}', 
           title='Interactive ${selectedMetric.charAt(0).toUpperCase() + selectedMetric.slice(1)} Chart')` : ''}
${chartType === 'line' ? `fig = px.line(df, x='timestamp', y='${selectedMetric}',
            title='Interactive ${selectedMetric.charAt(0).toUpperCase() + selectedMetric.slice(1)} Trend')` : ''}
${chartType === 'pie' ? `fig = px.pie(df, values='${selectedMetric}', names='category',
           title='Interactive ${selectedMetric.charAt(0).toUpperCase() + selectedMetric.slice(1)} Distribution')` : ''}
${chartType === 'scatter' ? `fig = px.scatter(df, x='value', y='${selectedMetric}',
               title='Interactive ${selectedMetric.charAt(0).toUpperCase() + selectedMetric.slice(1)} Scatter Plot')` : ''}
fig.show()`}
            </pre>
          </div>
        </div>
      ...
      <div className="bars-container">
        {data.slice(0, 8).map((item, index) => (
          <div key={index} className="bar-group">
            <div className="bar-label">{item.category || item.month || `Item ${index + 1}`}</div>
          ...
          {data.slice(0, 6).map((item, index) => {
            ...
            return (
              <g key={index}>
                ...
                />
                <text
                  x={150 + 100 * Math.cos(((startAngle + endAngle) / 2 * Math.PI) / 180)}
                  y={150 + 100 * Math.sin(((startAngle + endAngle) / 2 * Math.PI) / 180)}
                  textAnchor="middle"
                  fontSize="12"
                  fill="#374151"
                >
          ...
          {data.slice(0, 6).map((item, index) => (
            <div key={index} className="legend-item">
              <div 
                ...
                style={{ backgroundColor: `hsl(${240 + index * 40}, 70%, 60%)` }}
              ></div>
